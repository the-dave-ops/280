    ...
    <div className="card">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="text-center p-1.5 bg-blue-50 rounded-lg">
          ...
          <div className="text-lg font-bold text-blue-700">{Math.round(totalAmountToPay).toLocaleString('he-IL')} ₪</div>
        </div>
        <div className="text-center p-1.5 bg-green-50 rounded-lg">
          <div className="text-[10px] text-gray-600 mb-0.5">סה"כ שולם</div>
          <div className="text-lg font-bold text-green-700">{Math.round(totalPaid).toLocaleString('he-IL')} ₪</div>
        </div>
        <div className="text-center p-1.5 bg-red-50 rounded-lg">
          <div className="text-[10px] text-gray-600 mb-0.5">יתרה</div>
          <div className="text-lg font-bold text-red-700">{Math.round(totalBalance).toLocaleString('he-IL')} ₪</div>
        </div>
      </div>
    </div>
  );
